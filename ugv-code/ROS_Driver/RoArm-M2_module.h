...
double calculatePosByRad(double radInput) { return round((radInput / (2 * M_PI)) * ARM_SERVO_POS_RANGE); }
...
void servoTorqueCtrl(byte servoID, u8 enableCMD) { st.EnableTorque(servoID, enableCMD); }
...
{
    while (servoFeedback[InputID - 11].pos < goalPosition - offSet || servoFeedback[InputID - 11].pos > goalPosition + offSet)
    {
...
void RoArmM2_initCheck(bool returnType)
{
    ...
    RoArmM2_initCheckSucceed = getFeedback(BASE_SERVO_ID, true) && getFeedback(SHOULDER_DRIVING_SERVO_ID, true) &&
                               getFeedback(SHOULDER_DRIVEN_SERVO_ID, true) && getFeedback(ELBOW_SERVO_ID, true);
    if (!returnType)
    {
...
double maxNumInArray()
{
    if (EEMode == 0)
    {
        double deltaPos[4] = {abs(goalX - lastX), abs(goalY - lastY), abs(goalZ - lastZ), abs(goalT - lastT) * 10};
    ...
    {
        double deltaPos[4] = {abs(goalX - lastX), abs(goalY - lastY), abs(goalZ - lastZ), abs(goalT - lastT) * 200};
        double maxVal      = deltaPos[0];
        for (int i = 0; i < (sizeof(deltaPos) / sizeof(deltaPos[0])); i++)
        {
            maxVal = max(deltaPos[i], maxVal);
...
int calculatePosByDeg(double degInput) { return round((degInput / 360) * ARM_SERVO_POS_RANGE); }
